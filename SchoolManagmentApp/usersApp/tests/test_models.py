...
from ..models import Profile, ClassUnit, Parent, Student
...
class ProfileTestCase(TestCase):
    ...
    def test_default_photo(self):
        self.assertEqual(self.profile.photo.url, '/media/users/avatars/musk.webp')
...
class ClassUnitTestCase(TestCase):
    def test_valid_class_unit(self):
        class_unit = ClassUnit.objects.create(start_year=2023, study_year=1, letter_mark='A')
        self.assertEqual(str(class_unit), 'Class 1A')

    def test_invalid_start_year(self):
        with self.assertRaises(ValidationError):
            class_unit = ClassUnit(start_year=2022, study_year=1, letter_mark='A')
            class_unit.full_clean()

    def test_invalid_study_year(self):
        with self.assertRaises(ValidationError):
            class_unit = ClassUnit(start_year=2023, study_year=0, letter_mark='A')
            class_unit.full_clean()

    def test_invalid_letter_mark(self):
        with self.assertRaises(ValidationError):
            class_unit = ClassUnit(start_year=2023, study_year=1, letter_mark='X')
            class_unit.full_clean()

    def test_unique_together_constraint(self):
        ClassUnit.objects.create(start_year=2023, study_year=1, letter_mark='A')
        
        with self.assertRaises(ValidationError):
            class_unit = ClassUnit(start_year=2023, study_year=1, letter_mark='A')
            class_unit.full_clean()
...
class ClassUnitTestCase(TestCase):
    def setUp(self):
        ...
        self.profile = Profile.objects.create(user=self.user, phone_number='123456789', account_type=Profile.PARENT)
        self.profile2 = Profile.objects.create(user=self.user2, phone_number='123456789', account_type=Profile.STUDENT)
    ...
    def test_str_representation(self):
        self.assertEqual(str(self.parent), f"Parent: {self.parent.user.user.first_name} {self.parent.user.user.last_name}'s profile")
...
class StudentTestCase(TestCase):
    ...
    def test_limit_choices(self):
        invalid_user = User.objects.create_user(username='invaliduser', password='testpassword')
        invalid_profile = Profile.objects.create(user=invalid_user, phone_number='987654321', account_type=Profile.PARENT)
        
        with self.assertRaises(ValidationError):
            student = Student.objects.create(user=invalid_profile, class_unit=self.class_unit, parent=self.parent)
            student.full_clean()
