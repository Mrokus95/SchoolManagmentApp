...
class HomeViewTest(TestCase):
    def setUp(self):
        ...
        self.parent_profile = Profile.objects.create(
            user=self.user, account_type=Profile.PARENT
        )
    ...
    def test_get_home_as_unauthenticated_user(self):
        ...
        self.assertContains(
            response, 
            '<button id="login-nav-btn" class="login-nav-btn">Login</button>'
        )
...
class CustomPasswordResetConfirmViewTest(TestCase):
    def setUp(self):
        ...
        self.password_reset_confirm_url = reverse(
            "password_reset_confirm", args=[self.uid, self.token]
        )
...
class RegistrationCompleteTest(TestCase):
    def setUp(self):
        self.admin = User.objects.create_user(
            username="testuser1", password="testpassword1", is_staff=True
        )
        self.admin_profile = Profile.objects.create(
            user=self.admin, account_type=Profile.ADMIN
        )
...
class StudentRegisterViewTest(TestCase):
    def setUp(self):
        ...
        self.parent_profile2 = Profile.objects.create(
            user=self.user, account_type=Profile.PARENT
        )
    ...
    def test_invalid_registration(self):
        ...
        for field_name, field_errors in response.context[
            "registration_form"
        ].errors.items():
            for error in field_errors:
                self.assertContains(response, str(error))
...
class EditUserDataViewTest(TestCase):
    def setUp(self):
        ...
        self.parent_profile = Profile.objects.create(
            user=self.user, phone_number="549200023", 
    ...
    def test_authenticated_user_can_access(self):
        ...
        self.assertIsInstance(response.context["profile_form"], 
                              ProfileEditForm)
