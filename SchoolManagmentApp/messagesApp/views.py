...
from .forms import ShortEmailForm, FullEmailForm
# Create your views here.

@login_required
def get_inbox(request):
    if request.method == 'GET':
        inbox_messages = Message.objects.filter(receiver=request.user.profile, is_delete_receiver=False).order_by('-id')
        inbox_messages_not_read = Message.objects.filter(receiver=request.user.profile, is_delete_receiver=False, is_read_receiver=False).count()
...
def get_outbox(request):
    if request.method == 'GET':
        outbox_messages = Message.objects.filter(sender=request.user.profile,is_delete_sender=False).order_by('-date')
        inbox_messages_not_read = Message.objects.filter(receiver=request.user.profile, is_delete_receiver=False, is_read_receiver=False).count()
...
def get_important(request):
    if request.method == 'GET':
        inbox_messages = Message.objects.filter(receiver=request.user.profile, is_delete_receiver=False, is_important=True).order_by('-date')
        inbox_messages_not_read = Message.objects.filter(receiver=request.user.profile, is_delete_receiver=False, is_read_receiver=False).count()
        for email in inbox_messages:
            email.is_sender = False 
        paginator = Paginator(inbox_messages, 10)
...
def get_sent_email_details(request, id):
    ...
    if request.method == 'GET':
        ...
        if email.sender.id == user.id:
            form = ShortEmailForm()
            inbox_messages_not_read = Message.objects.filter(sender=request.user.profile, is_delete_sender=False, is_read_sender=False).count()
            context = {'email': email,
                    'not_read': inbox_messages_not_read,
                    'form': form,}
...
def create_email(request):
    ...
    if request.method == 'POST':
        ...
        form = FullEmailForm()
        inbox_messages_not_read = Message.objects.filter(receiver=request.user.profile, is_delete_receiver=False, is_read_receiver=False).count()
        context = {'not_read': inbox_messages_not_read,
                'form': form,}
