...
from ..models import User, Profile, Parent, ClassUnit
...
class RegistrationFormTest(TestCase):
    ...
    def test_invalid_phone_number(self):
            ...
            self.assertIn('Phone number has to be 9 digits.', form.errors['phone_number'])
    ...
    def test_invalid_phone_number_letters(self):
            ...
            self.assertFalse(form.is_valid())
            self.assertIn('phone_number', form.errors)
            self.assertIn('Phone number can only contains digits.', form.errors['phone_number'])
    ...
    def test_invalid_name(self):
        ...
        self.assertIn('First name can only contains alphabetical characters.', form.errors['first_name'])
    ...
    def test_invalid_name_surname(self):
        ...
        self.assertIn('Last name can only contains alphabetical characters.', form.errors['last_name'])
...
class TeacherRegistrationFormTest(TestCase):
    ...
    def test_valid_form_mulitple_subject_mixed(self):
        ...
        available_subjects = Subject.objects.all()
        form_data = {
            'name': [available_subjects[0].id, available_subjects[1].id, invalid_subject],
        }
        form = TeacherRegistrationForm(data=form_data)
        self.assertFalse(form.is_valid())
        self.assertIn('name', form.errors)
        self.assertIn('Some selected subjects are not valid.', form.errors['name'])
    ...
    def test_invalid_object(self):
        ...
        self.assertFalse(form.is_valid())
        self.assertIn('name', form.errors)
        self.assertIn('“invalid_subject_object” is not a valid value.', form.errors['name'])
...
class StudentRegistrationFormTest(TestCase):
    ...
    def setUp(self):
        ...
        self.class_unit = ClassUnit.objects.create(start_year = 2023, letter_mark='A')
...
class ProfileEditFormTest(TestCase):
    ...
    def test_invalid_form_with_letters(self):
        ...
        form = ProfileEditForm(data=form_data)
        self.assertFalse(form.is_valid())
        self.assertIn('Phone number can only contain digits.', form.errors['phone_number'])
